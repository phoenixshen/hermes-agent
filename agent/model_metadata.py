...
def is_grok_46_family(model: str) -> bool:
    """Whether *model* is Grok 4.6/4.7 family (same effort vocab + priority tier)."""
    name = (model or "").strip().lower().replace("_", "-").rsplit("/", 1)[-1]
    return (
        name == "grok-4.6" or name.startswith("grok-4.6-")
        or name == "grok-4.7" or name.startswith("grok-4.7-")
    )
